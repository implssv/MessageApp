class MessagesController < ApplicationController
  ...
  def index
    if(params.has_key?(:search))
      ...
      elseif(params.has_key?(:category))
       @messages = Message.joins(:category).where('category.name = ?', params[:category]).order("created_at DESC").paginate(:page => params[:page], :per_page => 9)
    else
